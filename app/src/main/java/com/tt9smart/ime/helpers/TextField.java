...
public class TextField extends InputField {
	...
	// Reads synchronously from the main thread — guarantees ordering with a prior setComposingText
	// call (both go through the same binder thread to the connected app).
	@NonNull public String getStringBeforeCursorSync(int numberOfChars) {
		CharSequence chars = numberOfChars > 0 ? InputConnectionAsync.getTextBeforeCursor(true, getConnection(), numberOfChars, 0) : null;
		if (chars == null || InputConnectionAsync.TIMEOUT_SENTINEL.equals(chars.toString())) return "";
